...
var GameForest                              = function (gameId, lobbyId, sessionId)
{
    ...
    this.playerAtIndex          = function (index)
    {
        return _sUserList[index];
    };

    this.userCount              = function ()
    {
        return _sUserList.length;
    };
    ...
    this.wsOnClose              = function (gfxObject)
    {
        ...
        {
            console.log("GAMEFOREST IS DISCONNECTED!");
    ...
    this.wsOnMessage            = function (gfxObject, message)
    {
        ...
        {
            console.log("Ping message received! Replying...");
        ...
        switch (parse.Subject)
        {
            ...
            case GFX_USER_DATA_CHANGED:
                ...
                break;
            case GFX_PING_RESPOND:

                console.log("Server says okay!");
                break;
            default:

                console.log("Rogue message received: " + message.data);
                break;
        }
    };
    ...
    this.askForReconnection     = function ()
    {
        ...
        sendRequest("/user/login?usersessionid=" + this.sessionId, "PUT",
            function (result)
            {
                ...
                thus.wsConnection = new WebSocket("ws://" + GameForestCloudUrl + ":8084");

                thus.wsConnection.onopen    = function () { thus.wsOnOpen(thus); };
                thus.wsConnection.onclose   = function () { thus.wsOnClose(thus); };
                thus.wsConnection.onmessage = function (message) { thus.wsOnMessage(thus, message); };

                constructWSRequest(thus.wsConnection, thus.connectionId, GFX_INFORM_RECONNECT, "");
            },
            function (status, why)
            {
                // if the user is playing and this suddenly went up. we know the user is disconnected from the internet.
